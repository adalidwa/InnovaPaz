  ...
  static async markAsConverted(cotizacionId, empresaId) {
    ...
    try {
      ...
      const countResult = await client.query(
        `SELECT COUNT(*) as total FROM pedidos WHERE empresa_id = $1`,
        [empresaId]
      );
      const numeroPedido = `PED-${new Date().getFullYear()}-${String(parseInt(countResult.rows[0].total) + 1).padStart(3, '0')}`;
